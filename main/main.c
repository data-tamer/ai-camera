...
// Task สำหรับส่งภาพผ่าน MQTT เฉพาะตอนมีการเคลื่อนไหว
void mqtt_image_task(void *pvParameter) {
    ESP_LOGI(TAG, "MQTT image task with motion detection started");
    
    while (1) {
        camera_fb_t *fb = esp_camera_fb_get();
        if (fb) {
            // ตรวจสอบการเคลื่อนไหวก่อนส่ง
            bool has_motion = detect_motion(fb);
            
            if (has_motion) {
                ESP_LOGI(TAG, "Motion detected - sending image via MQTT");
                mqtt_send_image(fb->buf, fb->len);
                
                // รอ 2 วินาทีหลังส่งภาพเพื่อป้องกันการส่งภาพติดต่อกันเร็วเกินไป
                vTaskDelay(2000 / portTICK_PERIOD_MS);
            } else {
                ESP_LOGI(TAG, "No motion detected - skipping MQTT send");
            }
            
            esp_camera_fb_return(fb);
        } else {
            ESP_LOGW(TAG, "Failed to capture camera frame");
        }
        
        // ตรวจสอบทุก 500ms สำหรับการเคลื่อนไหว (ปรับได้ตามต้องการ)
...
void app_main(void)
{
  ///> Initialize NVS
	...
	if (camera_init() == ESP_OK) {
		...
		// เริ่มต้น true motion detection ด้วยการเปรียบเทียบภาพ
		set_motion_detection(true);
		set_motion_cooldown(2000);   // 2 วินาที cooldown หลัง detect motion
		set_motion_threshold(5000);  // 5000 pixels เปลี่ยนแปลง = motion (ไวขึ้น)
		set_motion_pixel_threshold(25); // แต่ละ pixel ต้องแตกต่าง > 25
		ESP_LOGI(TAG, "True motion detection initialized - pixel comparison algorithm");
		
		xTaskCreate(&mqtt_image_task, "mqtt_image_task", 8192, NULL, 5, NULL); // เพิ่ม stack size
	}
}